...
import cc.lzsou.media.adapter.FolderAdapter;
import cc.lzsou.media.adapter.MediaGridAdapter;
import cc.lzsou.media.adapter.SpacingDecoration;
import cc.lzsou.media.data.DataCallback;
import cc.lzsou.media.data.ImageLoader;
import cc.lzsou.media.data.MediaLoader;
import cc.lzsou.media.data.VideoLoader;
import cc.lzsou.media.entity.Folder;
import cc.lzsou.media.entity.Media;
import cc.lzsou.media.utils.CropUtils;
import cc.lzsou.media.utils.ScreenUtils;
...
public class PickerActivity extends AppCompatActivity implements DataCallback, View.OnClickListener {
    ...
    void createFolderAdapter() {
        ...
        mFolderPopupWindow.setListSelector(null);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 200) {
            ArrayList<Media> selects = data.getParcelableArrayListExtra("data");
        ...
        if (requestCode == PickerConfig.REQUEST_CODE_TAKE_PHOTO && resultCode == RESULT_OK) {
            ArrayList<Media> selects = data.getParcelableArrayListExtra("data");
            done(selects);
        }
        if (requestCode == UCrop.REQUEST_CROP && resultCode == RESULT_OK) {
            ...
            ArrayList<Media> temp = new ArrayList<>();
            temp.add(new Media(resultUri.getPath(), "", 0, 1, 0, 0, ""));
            done(temp);
        }
    }
